...
import com.baustem.bndservice.socket.HTTPRequest;
...
public class DataModelHelperImpl implements DataModelHelper
{
    ...
    public Repository repository(final URL url) throws Exception
    {
        ...
        try
        {
            ...
            {
                ZipInputStream zin = new ZipInputStream(HTTPRequest.getBySocketInputStream(url, null));
            ...
            {
                is = new GZIPInputStream(HTTPRequest.getBySocketInputStream(url, null));                    
            }
            else
            {
                is = HTTPRequest.getBySocketInputStream(url, null);
            }
    ...
    public Resource createResource(final URL bundleUrl) throws IOException
    {
        ResourceImpl resource = createResource(new Headers()
        {
            ...
            private byte[] loadEntry(String name) throws IOException
            {
                InputStream is = null;
				try {
					is = HTTPRequest.getBySocketInputStream(bundleUrl, null);
				} catch (Exception e1) {
					e1.printStackTrace();
				}
                try
