...
public class ActivateSG extends HttpServlet {
	
	private static final long serialVersionUID = -3132676658906786107L;
	private static Log log = LogFactory.getLog(ActivateSG.class);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		case 32:
			result = getResult("32","symbolicName:"+sgname+" activate ok!");
			out.write(result);
			log.info(result);
			break;
		case 2:
			result = getResult("2", "symbolicName:"+sgname+"的Bundle 已经安装但还没有被解析");
			out.write(result);
			log.info(result);
			break;
		case 4:
			result = getResult("4", "symbolicName:"+sgname+"的Bundle 已经被解析而且可以被启动");
			out.write(result);
			log.info(result);
			break;
		case 8:
			result = getResult("8", "symbolicName:"+sgname+"的当前Bundle 正在启动的过程");
			out.write(result);
			log.info(result);
			break;
		case -1:
			result = getResult("-1", "symbolicName:"+sgname+"activate failed!");
			out.write(result);
			log.info(result);
		}
